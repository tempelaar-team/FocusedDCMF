...
def calc_QMdyn_get_QF(wavefn, Q, Qzp, lambda_al):
    ...
    for n in range(num_atom):
        Hq = H_q(Q, Qzp, wavefn[2*n: 2*n+int(2)], lambda_al[:,n])
        now_wavefn, now_wavefn_adb = prop_Q_exact(Hq, wavefn[2*n: 2*n+int(2)], dt)
...
def H_q(Q, Qzp, wavefn, lambda_alpha):
    Hq = np.zeros((2, 2, num_trj))
    for i in range(2):#ini_wavefn.size):
        Hq[i, i, :] = energy[i]
        for j in range(i+1, 2):#ini_wavefn.size):
            intQ = Q + np.abs(wavefn[j]) ** 2 * Qzp
...
#@jit(nopython=True)
def get_dipole(wavefn, groundwavefn):
    mu0mut = mu12**2 * (np.conjugate(wavefn[0]) * groundwavefn[1] + np.conjugate(wavefn[1]) * groundwavefn[0])
    return mu0mut
...
def run_dyn_propogation(ini_wavefn, Q, P, Qzp, Pzp, lambda_al):
    ...
    wavefnsquare_save, wavefnsquare_save_adb, sys_energy_save, \
            ...
            save_things(0, wavefn, wavefn_adb, muwavefn0, Q, P, Qzp, Pzp, Qzp_noQF, lambda_al, E0, Ezp0, E2zp0, Q0, Qzp0,
    ...
    for t in range(1, maxstep):
        ...
        P, Q, Pzp, Qzp = RK4_new(wgrid, P, Q, Pzp, Qzp, QF, QFzp, dt)
        #P_noQF, Q_noQF, Pzp_noQF, Qzp_noQF = RK4_new(wgrid, P_noQF, Q_noQF, Pzp_noQF, Qzp_noQF, np.zeros_like(QF), np.zeros_like(QF), dt)

        # saving data
        if t % savestep == 0:
            muwavefn = muwavefn0 * np.exp(-1j * energy[0] * t*dt)
            wavefnsquare_save, wavefnsquare_save_adb, sys_energy_save, \
            dipole_save, Nph_save, Nphzp_save, E2_save, E2zp_save, E0Etall_save, Q0Qtall_save = \
